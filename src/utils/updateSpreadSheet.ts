...
export default async function updateSpreadSheet(
  ...
  console.log('GOOGLE_SERVICE_ACCOUNT_EMAIL ', removeQuotes(process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || ''));
  console.log('GOOGLE_PRIVATE_KEY', removeQuotes(process.env.GOOGLE_PRIVATE_KEY || ''));

  // Initialize auth - see https://theoephraim.github.io/node-google-spreadsheet/#/guides/authentication
  const serviceAccountAuth = new JWT({
    // env var values here are copied from service account credentials generated by google
    // see "Authentication" section in docs for more info
    email: `${removeQuotes(process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL || '')}`,
    key: `${removeQuotes(process.env.GOOGLE_PRIVATE_KEY || '')}`,
    scopes: [
